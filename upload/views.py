...
class UploadView(LoginRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        objects = Document.objects.filter(user = request.user)
    ...
    def form_valid(self, form):
        for each in form.cleaned_data['attachments']:
            ...
            if (not Document.objects.filter(file_attached=file_path)):
                Document.objects.create(file_attached=each, user = self.request.user)
